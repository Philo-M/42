...
/*   Updated: 2022/01/05 15:57:58 by pmandel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	rest[fd] = rest_feed(rest[fd], fd);
	if (rest[fd] == NULL)
	{
		free (rest[fd]);
		return (NULL);
	}
	rest_before_n = rest_before(rest[fd]);
	rest_after_n = rest_after(rest[fd]);
	free (rest[fd]);
	rest[fd] = rest_after_n;
