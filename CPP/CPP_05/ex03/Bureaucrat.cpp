...
/*   Updated: 2022/08/04 17:30:25 by pmandel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include <iostream>
	...
	Bureaucrat::Bureaucrat(std::string const theName, int theGrade): _name(theName)
	{
		...
		{
			_grade = theGrade;
			std::cout << "Bureaucrat Constructor with these name: " << _name << " and grade: "
					<< _grade << std::endl;
		}
		else if (theGrade < 1)
		{
	...
	void	Bureaucrat::signForm(AForm & theForm)
	{
		try
		{
			...
			std::cout << _name << " signed " << theForm.getName() << std::endl;
		}
		catch(const std::exception& e)
		{
			std::cout << _name << " couldn\'t sign " << theForm.getName() <<
					" because: ";
	...
	void	Bureaucrat::executeForm(AForm const & theForm)
	{
		try
		{
			...
			std::cout << this->_name << " executed " << theForm.getName() << std::endl;
		}
		catch (const std::exception& e)
		{
			std::cout << this->_name << " couldn't execute " << theForm.getName() <<
					" because: ";
			std::cout << e.what() << std::endl;
		}
